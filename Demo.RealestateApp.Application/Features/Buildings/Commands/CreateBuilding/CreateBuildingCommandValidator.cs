...
namespace Demo.RealestateApp.Application.Features.Buildings.Commands.CreateBuilding
{
    public class CreateBuildingCommandValidator : AbstractValidator<CreateBuildingCommand>
    {
        ...
        public CreateBuildingCommandValidator(IBuildingRepository buildingRepository)
        {
            ...
            RuleFor(x => x)
               .MustAsync(BuildingNameUnique)
               .WithMessage("The Building already exists");
        ...
        private async Task<bool> BuildingNameUnique(CreateBuildingCommand x, CancellationToken token)
        {
            return !(await _buildingRepository.IsBuildingNameUnique(x.ProductTitle));
        }
    }
}
